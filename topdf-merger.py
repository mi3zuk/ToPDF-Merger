...
class ToPDFMerger(tk.Tk):
    ...
    def _cleanup_icon(self):
        if self._tmp_icon and os.path.exists(self._tmp_icon):
            try: os.remove(self._tmp_icon)
            except: pass
    ...
    def _build_ui(self):
        ...
        # bind scroll for PDF paging
        self.preview.bind("<MouseWheel>", self._on_scroll)
        self.preview.bind("<Button-4>",   self._on_scroll)
        self.preview.bind("<Button-5>",   self._on_scroll)

        tk.Button(right, text="PDF化して保存",
    ...
    def remove_selected(self):
        ...
        if size>0:
            new = idx if idx<size else size-1
    ...
    def sort_by_name(self):
        combo=list(zip(self.files,self.listbox.get(0,tk.END)))
        combo.sort(key=lambda x:
                   natural_key(os.path.basename(x[0])))
        self.files=[p for p,_ in combo]
        self.listbox.delete(0,tk.END)
        for _,nm in combo:
    ...
    def _on_drag_start(self,e):
        self._drag_start = self.listbox.nearest(e.y)
    def _on_drag_motion(self,e):
        i=self.listbox.nearest(e.y)
        j=self._drag_start
        if j is None or i==j: return
        self.files[i],self.files[j]=self.files[j],self.files[i]
    ...
    def _on_select(self,event):
        ...
        if path.lower().endswith('.pdf'):
            try:
                self._current_pdf = fitz.open(path)
                self._current_img = None
                self._pdf_page_index = 0
                self._draw_preview()
            except Exception as e:
                messagebox.showerror("PDFプレビューエラー", str(e))
        else:
            # image
            try:
                img = Image.open(path)
    ...
    def _on_canvas_resize(self,e):
        if self._current_img or self._current_pdf:
            self._draw_preview()

    # handle scroll for PDF pages
    def _on_scroll(self,event):
        ...
        if delta>0:
            # up → prev
            self._pdf_page_index = max(0,
                self._pdf_page_index-1)
        else:
            last = self._current_pdf.page_count-1
            self._pdf_page_index = min(last,
                self._pdf_page_index+1)
        self._draw_preview()
    ...
    def _draw_preview(self):
        ...
        # PDF preview
        elif self._current_pdf:
            page = self._current_pdf.load_page(self._pdf_page_index)
            rect = page.rect
            r = min(cw/rect.width, ch/rect.height)
            mat = fitz.Matrix(r,r)
            pix = page.get_pixmap(matrix=mat, alpha=False)
            img = Image.frombuffer("RGB",
                    (pix.width,pix.height),
                    pix.samples, "raw", "RGB", 0, 1)
            tkimg = ImageTk.PhotoImage(img)
        ...
        try:
            for p in self.files:
                if p.lower().endswith('.pdf'):
                    ...
                    buf.seek(0)
                    rdr2 = PdfReader(buf)
                    for pg in rdr2.pages:
                        writer.add_page(pg)
            with open(out,'wb') as f:
                writer.write(f)
            messagebox.showinfo("完了",
                f"{os.path.basename(out)} を生成しました")
        except Exception as e:
